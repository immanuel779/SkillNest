...
import {
  Building2, Save, Upload, AlertCircle, CheckCircle2,
} from 'lucide-react'
...
export default function EmployerCompanyProfile() {
  ...
  const onLogo = async (e) => {
    ...
    setError('')
    try {
      const url = await uploadFile(`logos/${user.uid}/${Date.now()}_${file.name}`, file)
      update('logoUrl', url)
    } catch (err) {
    ...
    <div className="container-app py-10 pb-32 max-w-3xl">
      ...
      <div className="card mb-6">
        <div className="flex items-center gap-5">
          <div className="w-20 h-20 rounded-2xl bg-brand-gradient flex items-center justify-center overflow-hidden shadow-lg shadow-brand-500/30">
            {form.logoUrl ? (
